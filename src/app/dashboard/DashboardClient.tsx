"use client";
import { Constants, useGlobalStore } from "@/store/useGlobalStore";
...
export default function DashboardClient({
  ...
  initialConstants: Partial<Constants>;
  initialData: [];
}) {
  ...
  useEffect(() => {
    ...
    setFilters({ ...filters, sector: initialConstants.sectors![0] });
    setData(initialData);
  }, []);
    ...
    fetchConstants(filters as Record<string, string | string[]>).then(
      (item) => {
        ...
        setConstants({
          ...
          pestles: item.pestles,
          sources: item.sources,
          regions: item.regions,
          topics: item.topics,
          end_years: item.end_years,
        });
      }
    );
  }, []);
